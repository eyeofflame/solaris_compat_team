...
public class ScriptRegistry {
     ...
     * 注册内置的示例剧本 {@code test}。
     *
     * <p>结构：旁白开场 → 奥利维亚问话 → 二选一分支 → 两条分支汇合到同一个节点 → 结尾。
     * 结尾节点的 {@code nextId} 是 {@code ""}，表示"没有下一节点"，玩家再点一下界面就关掉。
     */
    public static void defaultReg() {
        List<Node> nodes = new ArrayList<>();

        nodes.add(line("start", "", "你推开酒馆的门，木地板在脚下吱呀作响。", "greet"));
        nodes.add(line("greet", "奥利维亚", "哟，稀客。这么晚了还来我这儿——是有事，还是单纯想喝酒？", "branch"));
        ...
        nodes.add(new Node("branch", "奥利维亚", "……说吧，我听着。", choices, null));
